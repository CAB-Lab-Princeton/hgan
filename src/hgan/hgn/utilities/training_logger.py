...
class TrainingLogger:
    def __init__(
        ...
        self.writer = SummaryWriter(
            log_dir=os.path.join(log_dir, hyper_params["experiment_id"])
        )
    ...
    def step(self, losses, rollout_batch, prediction, model):
        ...
        if self.iteration % self.loss_freq == 0:
            ...
            self.writer.add_scalar("energy/mean", enery_mean, self.iteration)
            self.writer.add_scalar("energy/std", energy_std, self.iteration)

        if self.iteration % self.rollout_freq == 0:
            self.writer.add_video(
                "data/input", rollout_batch.detach().cpu(), self.iteration
            )
            self.writer.add_video(
                "data/reconstruction",
        ...
        if self.iteration % self.model_freq == 0:
            save_dir = os.path.join(
                self.hparams["model_save_dir"],
                self.hparams["experiment_id"] + "_checkpoint_" + str(self.iteration),
            )
            model.save(save_dir)
        self.iteration += 1
