...
class HGN:
    ...
    def forward(self, rollout_batch, n_steps=None, variational=True):
        ...
        prediction = HgnResult(
            batch_shape=torch.Size(prediction_shape), device=self.device
        )
        ...
        for _ in range(n_steps):
            ...
            prediction.append_state(q=q, p=p)
            prediction.append_energy(
                self.integrator.energy
    ...
    def load(self, directory):
        ...
        self.encoder = torch.load(
            os.path.join(directory, self.ENCODER_FILENAME), map_location=self.device
        )
        self.transformer = torch.load(
            os.path.join(directory, self.TRANSFORMER_FILENAME), map_location=self.device
        )
        self.hnn = torch.load(
            os.path.join(directory, self.HAMILTONIAN_FILENAME), map_location=self.device
        )
    ...
    def get_random_sample(self, n_steps, img_shape=(32, 32)):
        ...
        latent_shape = (
            1,
            self.encoder.out_mean.out_channels,
            img_shape[0],
            img_shape[1],
        )
