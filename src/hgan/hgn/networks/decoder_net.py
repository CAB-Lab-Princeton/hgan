...
class DecoderNet(nn.Module):
    ...
    def __init__(
        ...
        if all(var is None for var in (n_residual_blocks, n_filters, kernel_sizes)):
            ...
            var is not None for var in (n_residual_blocks, n_filters, kernel_sizes)
        ):
            assert len(kernel_sizes) == n_residual_blocks + 1, (
                "kernel_sizes and upsample must be of length n_residual_blocks + 1 ("
                + str(n_residual_blocks + 1)
                + " in this case)."
            )
            assert len(n_filters) == n_residual_blocks, (
                "n_filters must be of length "
