...
class HamiltonianNet(nn.Module):
    ...
    def __init__(
        ...
        if all(
            var is None
            for var in (hidden_conv_layers, n_filters, kernel_sizes, strides, paddings)
        ):
            ...
            assert (
                len(kernel_sizes) == hidden_conv_layers + 2
                and len(strides) == len(kernel_sizes)
                and len(paddings) == len(kernel_sizes)
            ), (
        ...
        self.out_conv = nn.Conv2d(
            in_channels=n_filters[-1], out_channels=1, kernel_size=2, padding=0
        )
    ...
    def forward(self, q, p):
        ...
        x = torch.cat(
            (q, p), dim=1
        )  # Concatenate q and p to obtain a N x 2C x H x W tensor
