...
from networks import decoder_net  # noqa: E402
from networks import encoder_net  # noqa: E402
from networks import hamiltonian_net  # noqa: E402
from networks import transformer_net  # noqa: E402
...
class EncoderNet(encoder_net.EncoderNet):
    ...
    def __init__(self, phi=None, dtype=torch.float):
        ...
        self.phi = (
            phi
            if phi is not None
            else nn.Parameter(torch.tensor([1.0, 2.0], requires_grad=True, dtype=dtype))
        )
...
class TransformerNet(transformer_net.TransformerNet):
    ...
    def __init__(self, w=None, dtype=torch.float):
        ...
        self.w = (
            w
            if w is not None
            else nn.Parameter(torch.tensor([1.0, 1.0], requires_grad=True, dtype=dtype))
        )
...
class HamiltonianNet(hamiltonian_net.HamiltonianNet):
    ...
    def __init__(self, gamma=None, dtype=torch.float):
        ...
        self.gamma = (
            ...
            else nn.Parameter(torch.tensor([3.0, 4.0], requires_grad=True, dtype=dtype))
        )
...
class DecoderNet(decoder_net.DecoderNet):
    ...
    def __init__(self, theta=None, dtype=torch.float):
        ...
        self.theta = (
            ...
            else nn.Parameter(torch.tensor([2.0], dtype=dtype))
        )
