...
class ChaoticPendulum(Environment):
    ...
    def _dynamics(self, t, states):
        ...
        dyn[0, 0] = states_resh[1, 0] - states_resh[1, 1] * np.cos(
            states_resh[0, 0] - states_resh[0, 1]
        )
        dyn[0, 1] = states_resh[1, 1] - states_resh[1, 0] * np.cos(
            states_resh[0, 0] - states_resh[0, 1]
        )
        dyn[0, :] /= quot
        # dp_1 and dp_2
    ...
    def _draw(self, res=32, color=True):
        ...
        for t in range(length):
            ...
            vid[t] = cv2.circle(
                vid[t], coords_1, int(self.length / (space_res * 3)), ball_colors[0], -1
            )
            vid[t] = cv2.circle(
                vid[t], coords_2, int(self.length / (space_res * 3)), ball_colors[1], -1
            )
    ...
    def _sample_init_conditions(self, radius):
        ...
        """
        states_q = np.random.rand(2) * 2.0 - 1
        states_q = (states_q / np.sqrt((states_q**2).sum())) * radius
        states_p = np.random.rand(2) * 2.0 - 1
...
if __name__ == "__main__":
    ...
    pd = ChaoticPendulum(mass=1.0, length=1, g=3)
