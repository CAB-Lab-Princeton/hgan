...
from utilities import conversions  # noqa: E402
...
class EnvironmentLoader(Dataset):
    ...
    def __getitem__(self, i):
        rolls = np.load(os.path.join(self.root_dir, self.file_list[i]))["arr_0"]
...
if __name__ == "__main__":
    import time
    from hgan.hgn.environments.pendulum import Pendulum
    ...
    train = torch.utils.data.DataLoader(trainDS, shuffle=False, batch_size=None)
    start = time.time()
    ...
    train = torch.utils.data.DataLoader(
        trainDS, shuffle=False, batch_size=10, num_workers=4
    )
    start = time.time()
    sample = next(iter(train))
    end = time.time()
