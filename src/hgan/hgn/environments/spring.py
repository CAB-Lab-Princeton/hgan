...
class Spring(Environment):
    ...
    WORLD_SIZE = 2.0
    PHYSICAL_PROPERTIES = ("mass", "elastic_cst", "damping_ratio")

    def __init__(self, mass=0.5, elastic_cst=2.0, damping_ratio=0.0, q=None, p=None):
    ...
    def _draw(self, res=32, color=True, constant_color=True):
        ...
        for t in range(length):
            vid[t] = cv2.circle(
                vid[t],
                self._world_to_pixels(0, q[t], res),
                int(self.mass / space_res),
                ball_color,
                -1,
            )
...
if __name__ == "__main__":
    ...
    sp = Spring(mass=0.5, elastic_cst=2, damping_ratio=0.0)
